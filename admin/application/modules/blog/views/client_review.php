               <!-- START BREADCRUMB -->
                <ul class="breadcrumb">
                    ...
                    <li><a href="<?php echo site_url('add-tour-package');?>">Tour Package</a></li>
                    <li class="active">Tour Package List</li>
                </ul>
                <!-- END BREADCRUMB -->
                <?php
                                ...
                                <div class="panel-body">
								   ...
								   <form id="form1"  method="post" action="<?php echo site_url("blog/delete_all_review");?>">
                                    <table id="customers2" class="table table-striped table-actions datatable">
                                        ...
                                        <tbody>
										<?php if(!empty($get_review) ) { ?>
										<?php foreach($get_review as $key=>$get_reviews):
										//printarray($get_reviews);
										?>
                                            <tr>
                                               
                                                <td><?php echo $key+1;?></td>
												<td style="padding: 0;"><label class="check"><input type="checkbox" name="checklist[]" value="<?php echo $get_reviews->id; ?>" class="checkbox1"/></label></td>
                                                <td><?php echo $get_reviews->cust_name;?></td>
                                                <td><?php echo $get_reviews->cust_email;?></td>
                                                <td>
												<button type="button" class="btn btn-info btn-sm" data-toggle="modal" data-target="#myModal_<?php echo $key; ?>"> View Comment </button>
												</td>
                                                <td><?php echo $get_reviews->custmore_phone;?></td>
                                                <td><?php if($get_reviews->publish_unpublish=="publish"){ ?>

												<i  class='btn btn-danger btn-rounded btn-sm' onClick='Deactive_package("<?php echo $get_reviews->id;?>")' style='color: green;font-size: 16px; background:green; color:white'>Publish</i>

												<?php } else { ?>
												<i class='btn btn-danger btn-rounded btn-sm' onClick='Deactive_package("<?php echo $get_reviews->id;?>")'  style='color: #B64645;font-size: 16px; background:red; color:white'>Unpublish</i>
												<?php } ?></td>
                                                <td>
                                                 <?php $packagekey=$get_reviews->id;?>
				...
				<!-- Modal  FORM POP UP START HERE-->
									<?php foreach($get_review as $mod=>$get_reviews){ ?>
								<div id="myModal_<?php echo $mod; ?>" class="modal fade" role="dialog">
								  <div class=" btn-rounded btn-sm modal-dialog">

									<!-- Modal content-->
									<div class="modal-content">
									  <div class="modal-header">
										<button type="button" class="close" data-dismiss="modal">&times;</button>
										<h4 class="modal-title">Decription Comment</h4>
									  </div>
									  <div class="modal-body">
										<p class="review-txt"><?php  echo $get_reviews->custmore_review; ?></p>
									  </div>
