(function($) {
	...
	//Testimonial Carousel (legacy layout on inner pages)
	if ($('.testimonial-carousel').not('.dontia-testimonial-carousel').length) {
		$('.testimonial-carousel').not('.dontia-testimonial-carousel').owlCarousel({
			loop:true,
			margin:30,
			nav:true,
			smartSpeed: 700,
...
/* ==========================================================================
   ...
   ========================================================================== */
	...
	/* Home — Latest Technology: tap to flip on touch / small screens */
	$(document).on('click', '.dontia-tech-tile', function (e) {
		if (window.matchMedia('(hover: hover) and (pointer: fine)').matches && $(window).width() >= 768) {
			return;
		}
		e.preventDefault();
